...
function product_card_options() {
    ...
    $options['image_ratio'] = in_array( $options['image_ratio'], array( 'portrait', 'square', 'landscape' ), true ) ? $options['image_ratio'] : 'portrait';
    $options['content_order'] = in_array( $options['content_order'], array( 'title-price', 'price-title' ), true ) ? $options['content_order'] : 'title-price';
    $options['content_align'] = in_array( $options['content_align'], array( 'left', 'center' ), true ) ? $options['content_align'] : 'left';
    $options['price_treatment'] = in_array( $options['price_treatment'], array( 'standard', 'emphasis', 'muted' ), true ) ? $options['price_treatment'] : 'standard';
    $options['action_treatment'] = in_array( $options['action_treatment'], array( 'button', 'outline', 'text' ), true ) ? $options['action_treatment'] : 'button';
    $options['hover_behavior'] = in_array( $options['hover_behavior'], array( 'none', 'lift', 'image-zoom' ), true ) ? $options['hover_behavior'] : 'lift';
    $options['badge_style'] = in_array( $options['badge_style'], array( 'pill', 'corner', 'minimal' ), true ) ? $options['badge_style'] : 'pill';
...
function render_product_state_badges() {
    ...
    if ( $options['show_state_badges'] ) {
        if ( ! $product->is_in_stock() ) {
            $badges['sold-out'] = array( 'label' => __( 'Sold out', 'itk-commerce' ), 'class' => 'sold-out' );
        }
        if ( $product->is_featured() ) {
            $badges['featured'] = array( 'label' => __( 'Featured', 'itk-commerce' ), 'class' => 'featured' );
        }
        $created = $product->get_date_created();
        if ( $created && $created->getTimestamp() >= ( time() - ( DAY_IN_SECONDS * absint( $options['new_days'] ) ) ) ) {
            $badges['new'] = array( 'label' => __( 'New', 'itk-commerce' ), 'class' => 'new' );
        }
    }
...
/**
 * Render the semantic optional action slots only when integrations are active.
 * Quick View, Wishlist and Compare retain dedicated hook names while the
 * original general product-card action hook remains supported last.
 */
function render_product_card_actions() {
    ...
    $has_actions = function_exists( __NAMESPACE__ . '\\product_card_has_actions' )
        ? product_card_has_actions()
        : (bool) has_action( 'itk_commerce_product_card_actions' );

    if ( ! $has_actions ) {
        return;
    }

    $context = product_card_context();
    echo '<div class="itk-product-card-actions">';
    if ( function_exists( __NAMESPACE__ . '\\render_product_card_action_hooks' ) ) {
        render_product_card_action_hooks( $product, $context );
    } else {
        do_action( 'itk_commerce_product_card_actions', $product, $context );
    }
    echo '</div>';
}
